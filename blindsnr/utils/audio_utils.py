...
def generate_noisy_signal(desired_snr, signal, noise, verbose=False):
    """
    ...
    verbose: Boolean flag to control verbosity (default is False)
    ...
    if verbose:
        ...
        print(f"True SNR: {calculate_true_snr(signal * a, noise * b):.2f} dB")
    return noisy_signal
...
def read_audio(func):
    ...
    def wrapper(input_wav, *args, **kwargs):
        ...
        if "sample_rate" in kwargs:
            sample_rate = kwargs.pop("sample_rate")  # Remove the sample rate from kwargs to avoid conflicts
